...
### Shipped broken once, then fixed: React #438 on the review page

The review page took `params: Promise<{id}>` and read it with `use(params)` — the **Next.js 15** idiom. This project is on **Next.js 14.2.35**, where a page's `params` is a plain object, and React's `use()` rejects a non-Promise. Result: parsing succeeded, `router.push` to the review page threw during render, and the whole screen went to "Application error: a client-side exception has occurred".

It typechecked and it built. The API routes were unaffected because `await params` on a non-thenable just returns the value — which is exactly why the backend pipeline verified healthy end-to-end while this one screen was dead. Now uses `useParams()`, matching every other dynamic admin page. [tests/next14-params-convention.test.ts](tests/next14-params-convention.test.ts) pins it repo-wide and also flags [app/admin/licensing/[id]/page.tsx](app/admin/licensing/[id]/page.tsx), which mistypes `params` the same way but consumes it with `await`, so it works today and is one refactor from the same crash.

### Not done yet

- **Deep UI pass** — the review screen loads and works in production. Still unexercised: ticking per-field apply checkboxes on a LIVE match end-to-end, the `reviewed_at` gate in anger, bulk select on a large batch, and the responsive/keyboard pass.
