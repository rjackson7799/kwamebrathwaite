...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const filters = adminPressFiltersSchema.safeParse(params)
    ...
    const { page, limit, type, status, featured, q, sort, order } = filters.data
    ...
    // Apply filters
    if (type) query = query.eq('press_type', type)
    if (status) query = query.eq('status', status)
    if (featured !== undefined) query = query.eq('is_featured', featured)
    if (q) query = query.or(`title.ilike.%${q}%,publication.ilike.%${q}%,author.ilike.%${q}%`)
    ...
    if (error) {
      ...
      return errorResponse(ErrorCodes.DB_ERROR, 'Failed to fetch press items', 500)
    }
...
// POST /api/admin/press - Create new press item
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (error) {
      console.error('Database error:', error)
      return errorResponse(ErrorCodes.DB_ERROR, 'Failed to create press item', 500)
    }

    // Log activity
    const userEmail = await getCurrentUserEmail()
    if (userEmail && data) {
      await logActivity(userEmail, 'create', 'press', data.id, data.title)
    }
